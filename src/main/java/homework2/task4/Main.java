...
/* *
 * Крестики-нолики поле 5х5 с изменённой проверкой на победу из 4-х одинаковых фишек с
 * искусственным интеллектом, который проверяет наличие трёх или двух одинаковых фишек у человека и
...
public class Main {
    ...
    static boolean checkDiUp(int x, int y, char dot, int winCount) {
        ...
        if (isCellValid(x + winCount - 1, y - winCount + 1)) {
            for (int i = 1; i < winCount; i++) {
                if (field[y][x] == dot && field[y - i][x + i] == dot) {
                    count++;
                }
            }
            if (count == winCount) {
    ...
    static boolean checkHo(int x, int y, char dot, int winCount) {
        ...
        if (isCellValid(x + winCount - 1, y)) {
            for (int i = 1; i < winCount; i++) {
                if (field[y][x] == dot && field[y][x + i] == dot) {
                    count++;
                }
            }
            if (count == winCount) {
    ...
    static boolean checkVe(int x, int y, char dot, int winCount) {
        ...
        if (isCellValid(x, y + winCount - 1)) {
            for (int i = 1; i < winCount; i++) {
                if (field[y][x] == dot && field[y + i][x] == dot) {
                    count++;
                }
            }
            if (count == winCount) {
        ...
        for (int y = 0; y < fieldSizeY; y++) {
            for (int x = 0; x < fieldSizeX; x++) {
                if (isCellEmpty(x, y)) {
                    ...
                    if (checkWin(DOT_HUMAN, winCount - 1)) {
                        field[y][x] = DOT_AI;
                        return true;
                    }
                    field[y][x] = DOT_EMPTY;
